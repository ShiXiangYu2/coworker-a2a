...
export async function POST(request: Request, ctx: RouteContext<'/api/a2a/messages/[id]/submit-review'>) {
  try {
    const { id } = await ctx.params
    const body = await readJson(request)
    const data = await reviewA2AMessage(id, 'SUBMIT_REVIEW', {
      reviewedBy: stringValue(body.reviewedBy),
      decisionReason: stringValue(body.decisionReason) ?? 'Submitted local A2A draft for review.',
    })
