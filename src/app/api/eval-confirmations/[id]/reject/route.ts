import { rejectEvalConfirmation } from '@/lib/eval/repository'
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const result = await rejectEvalConfirmation(id, {
      reviewedBy: stringValue(body.reviewedBy) ?? 'kelvin',
      decisionReason: stringValue(body.decisionReason) ?? 'Rejected local Eval review.',
    })
