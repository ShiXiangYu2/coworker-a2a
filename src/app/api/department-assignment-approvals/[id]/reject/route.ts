...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await transitionDepartmentAssignmentRecord({ recordType: 'department_assignment_approval_record', id, targetStatus: 'rejected', reason: 'Rejected local department assignment approval record only.' })
