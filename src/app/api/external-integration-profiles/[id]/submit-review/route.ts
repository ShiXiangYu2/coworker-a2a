import { updateExternalIntegrationProfileStatus } from '@/lib/external-mcp-governance/repository'
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const result = await updateExternalIntegrationProfileStatus(id, 'SUBMIT_REVIEW', stringValue(body.reason) ?? 'Submitted local integration profile for review.')
