...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await transitionDepartmentAssignmentRecord({ recordType: 'department_assignment_proposal', id, targetStatus: 'superseded', reason: 'Superseded local department assignment proposal only.' })
