...
export async function GET(
  ...
  try {
    ...
    const record = await getExecutionGatewayRecordById('execution_approval_record', id)
    if (!record) return Response.json({ ok: false, error: { code: 'not_found', message: 'Execution approval record not found.' } }, { status: 404 })
