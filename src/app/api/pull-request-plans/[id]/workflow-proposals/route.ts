/**
 * GET /api/pull-request-plans/:id/workflow-proposals — List workflow proposals linked to a PullRequestPlan
 */
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const proposals = await prisma.workflowProposal.findMany({
      where: { sourceRecordId: id, sourceKind: 'pull_request_plan' },
