...
export async function GET(
  ...
  try {
    ...
    const record = await getExecutionGatewayRecordById('execution_receipt_record', id)
    if (!record) return Response.json({ ok: false, error: { code: 'not_found', message: 'Execution receipt record not found.' } }, { status: 404 })
