...
export async function GET(request: Request) {
  try {
    ...
    const data = correlationId
      ? await listRuntimeExecutionRecordsByCorrelationId({ correlationId, limit })
      : await listRecentRuntimeExecutionRecords({ limit })
    ...
    return Response.json(
      {
        ok: false,
        error: {
          code: 'runtime_executions_query_error',
          message: error instanceof Error ? error.message : 'Unable to query runtime executions.',
        },
      },
      { status: 400 }
    )
