...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await transitionDepartmentAssignmentRecord({ recordType: 'department_role_fit_review', id, targetStatus: 'review', reason: 'Submitted local department role fit review for review only.' })
