...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await transitionDepartmentAssignmentRecord({ recordType: 'department_task_intake_record', id, targetStatus: 'rejected', reason: 'Rejected local department task intake record only.' })
