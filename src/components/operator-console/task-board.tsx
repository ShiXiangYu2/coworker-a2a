...
export function TaskBoard() {
  ...
  const filtered = filter === 'all' ? tasks : tasks.filter((bundle) => bundle.task.status === filter)
  const statusCounts = tasks.reduce(
    (acc, bundle) => {
      acc[bundle.task.status] = (acc[bundle.task.status] || 0) + 1
      return acc
    },
      ...
      <div className="max-h-96 overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {filtered.map((bundle) => (
              <div key={bundle.task.id} className="px-4 py-3 hover:bg-gray-50">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <StatusBadge status={bundle.task.status} />
                    <span className="break-words text-sm font-medium text-gray-900">{bundle.task.title}</span>
                  </div>
                  <div className="flex shrink-0 flex-wrap items-center gap-3 text-xs text-gray-500">
                    {bundle.task.targetAgentId && (
                      <span className="rounded bg-sky-50 px-2 py-0.5 text-sky-700">
                        {agentLabels[bundle.task.targetAgentId] ?? bundle.task.targetAgentId}
                      </span>
                    )}
                    <span>{(bundle.task.confidence * 100).toFixed(0)}%</span>
                    <span>{new Date(bundle.task.createdAt).toLocaleDateString()}</span>
                    <Link
                      href={`/operator?runtimeTaskId=${encodeURIComponent(bundle.task.id)}`}
